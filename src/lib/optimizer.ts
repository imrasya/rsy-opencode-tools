    ...
    if (Object.keys(byModel).length === 1 && entries.length > 10) {
      ...
      if (currentProfile && tier !== "cheap") {
        suggestions.push({
          ...
          reason: `All ${entries.length} requests use the same model. Enable smart routing ('rsy-opencode-tools route') to automatically select cheaper models for simple tasks.`,
        });
      }
    }
  }
