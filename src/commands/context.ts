...
const showCommand = new Command("show")
  ...
  .action(async () => {
    ...
    if (!existsSync(contextPath)) {
      ...
      info(`Run ${chalk.cyan("rsy-opencode-tools context init")} to create one.`);
      process.exit(EXIT_ERROR);
    }
...
const clearCommand = new Command("clear")
  ...
  .action(async (opts: { confirm?: boolean }) => {
    ...
    if (!opts.confirm) {
      ...
      warn(`Run with --confirm to proceed: ${chalk.cyan("rsy-opencode-tools context clear --confirm")}`);
      process.exit(EXIT_ERROR);
    }
...
const statusCommand = new Command("status")
  ...
  .action(async () => {
    ...
    if (!existsSync(contextPath)) {
      ...
      console.log();
      info(`Create one with: ${chalk.cyan("rsy-opencode-tools context init")}`);
      process.exit(EXIT_SUCCESS);
    }
