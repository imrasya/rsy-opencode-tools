...
export const doctorCommand = new Command("doctor")
  .description("Run a full health check of the RSY OpenCode Tools installation")
  ...
  .action(async (opts: { fix?: boolean; installTools?: boolean }) => {
    ...
    if (opts.fix && (errors > 0 || warnings > 0)) {
      ...
      if (fixResults.length === 0) {
        ...
        if (fixed > 0) {
          console.log();
          info("Run " + chalk.cyan("rsy-opencode-tools doctor") + " again to verify fixes.");
        }
      }
    ...
    if (!opts.fix && (errors > 0 || warnings > 0)) {
      console.log();
      info("Run " + chalk.cyan("rsy-opencode-tools doctor --fix") + " to auto-fix issues.");
    }
